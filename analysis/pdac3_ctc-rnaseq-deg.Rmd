---
title: "Analysis of PDAC-3 xenograft samples"
subtitle: "Differential gene expression between CTC types"
...
params:
  date: '`r format(Sys.Date(), "%B %d, %Y")`'
  se_path: ./data/rnaseq/pdac3-ctc/se_qc_hs.rds
  output_dir: ./output/rnaseq/pdac3-ctc/differential_expression_edger/sample_type
  min_counts: 5
...
knitr::opts_chunk$set(results='asis', echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, fig.align = 'center', dpi = 600, dev = c("png", "pdf"), engine.opts = list(bash = "-l"))
...
ctc_single_samples <- x %>% 
  filter(sample_type == 'ctc_single_pool') %>% 
  pull(sample_alias)
...
ctc_cluster_hetero_samples <- x %>% 
  ...
  pull(sample_alias)
cto_samples <- x %>% 
  filter(sample_type %in% c('cto', 'cto_heterotypic')) %>% 
  pull(sample_alias)
```
...
for(cn in names(comp_list)) {
  ...
  sample_samples <-  x %>% 
    filter(sample_alias %in% i[[1]]) %>% 
    filter(mouse_id %in% use_batch_levels) %>% 
    pull(sample_alias)
  ctrl_samples <-  x %>% 
    filter(sample_alias %in% i[[2]]) %>% 
    filter(mouse_id %in% use_batch_levels) %>% 
